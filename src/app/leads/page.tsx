...
export default async function LeadsPage({ searchParams }: { searchParams: Promise<{ status?: string; niche?: string }> }) {
  ...
  if (filters.niche) { values.push(filters.niche); where.push(`niche = $${values.length}`); }
  ...
  return (
    <div className="space-y-6">
      ...
      <form className="flex flex-wrap gap-3 rounded-2xl border border-slate-200 bg-white p-4">
        <select name="status" defaultValue={filters.status ?? ""} className="rounded-lg border border-slate-300 px-3 py-2 text-sm"><option value="">Todos os status</option>{["discovered","qualified","disqualified","dm_sent","replied","handed_off","converted","do_not_contact"].map((status) => <option key={status}>{status}</option>)}</select>
